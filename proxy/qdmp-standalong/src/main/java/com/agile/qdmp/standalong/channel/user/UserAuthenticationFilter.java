...
import com.agile.qdmp.standalong.utils.NetworkUtil;
...
public class UserAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private static final String SPRING_SECURITY_RESTFUL_SERVER = "server";
    private static final String SPRING_SECURITY_RESTFUL_LOGIN_URL = "/login/im";
    ...
    @Getter
    @Setter
    private String serverParameter = SPRING_SECURITY_RESTFUL_SERVER;
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) {
        ...
        NetworkUtil.ipList.add(ip);
//        System.out.println("===============> 登录IP：" + ip + ", NetworkUtil.ipList:" + NetworkUtil.ipList);
        if(NetworkUtil.ipList.size() > 5) {
            throw new AuthenticationServiceException("客户端使用数目超限");
        }
        ...
        String serverUri = obtainParameter(request, SPRING_SECURITY_RESTFUL_SERVER);
        if(StringUtils.isBlank(principal)) {
            ...
            if(StringUtils.isNotBlank(payload)) {
                ...
                serverUri = JSONObject.parseObject(payload).getString(SPRING_SECURITY_RESTFUL_SERVER);
            }
        }
        UserAuthenticationToken usernameAuthenticationToken = new UserAuthenticationToken(principal, credentials, serverUri);
